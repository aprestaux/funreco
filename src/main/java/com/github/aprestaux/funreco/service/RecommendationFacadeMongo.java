...
public class RecommendationFacadeMongo implements RecommendationFacade {
	...
	private String jsonIdToString(String jsonString) {
		...
		String id_str = idNode.getValueAsText();
		return id_str;
	}
	...
	private static List<DBAction> pickNRandom(Query<DBAction> lst, int n) {
		List<DBAction> copy = lst.asList();
		Collections.shuffle(copy);
		ArrayList<DBAction> subList = new ArrayList<DBAction>();
		for (int i = 0; i < copy.size(); i++) {
			if (i == n) {
				break;
			}
	...
	public Recommendations findRecommendationsByProperties(String... properties) {
		...
		Query<DBAction> q = datastore.find(DBAction.class);
		for (DBAction dbAction : q) {
			if (dbAction.getObject().containsValue(properties)
					&& !result.contains(dbAction)) {
				result.add(dbAction);
			}
		}
		...
		for (String friendId : friendsIds) {
			for (DBAction dbAction_tmp : allActionsOfProfile(friendId)) {
				for (DBAction actionOfProfile : allActionsOfProfile(id)) {
					if (!actionOfProfile.getObject().getObjectId()
							.equals(dbAction_tmp.getObject().getObjectId()))
						dbActions.add(dbAction_tmp);
				}
			}
		}
		...
		for (DBAction dbAction : dbActions) {
			...
			if (recommendedObjects.containsKey(object.getId())) {
				recommendedObjects.get(object.getId()).getBy()
						.add(recomendorId);
				if (!recommendedObjects.get(object.getId()).getByEmail()
						.contains(recomendorEmail))
					recommendedObjects.get(object.getId()).getByEmail()
							.add(recomendorEmail);
			} else {
	...
	@Override
	public List<RecommendedObject> getRecommendedObjects(Recommendations recos,
			int limit) {
		...
		Iterator<RecommendedObject> iterator = recommendation.getObjects()
				.iterator();
	...
	private DBProfile findByEmail(String email) {
		return datastore.find(DBProfile.class)
				.filter("attributes.email", email).get();
	}
	...
	private Query<DBAction> allActionsOfProfile(String id) {
		return datastore.find(DBAction.class).filter("profile.externalId", id);// .filter("profile.externalId",
																				// id);
	}
